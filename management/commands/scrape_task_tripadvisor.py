...
lockfile = '/tmp/scrape_task_tripadvisor_lock_file'
...
class Command(BaseCommand):
    help = "Scrape Task Tripadvisor"
    def add_arguments(self, parser):
	print 'add nothing'
    def handle(self, *args, **options):
        print 'scrape_task_tripadvisor.py - handle_noargs'
        now = datetime.now()
        print 'now: %s' % now
	...
	try:
		...
		interval = 1
	        hotels = Hotel.objects.filter(is_unavailable=False, is_tripadvisor_update_running=False, tripadvisor_last_update_date=None).order_by('tripadvisor_last_update_date','-hotel_id')
		if len(hotels) == 0:
		    hotels = Hotel.objects.filter(is_unavailable=False, is_tripadvisor_update_running=False).order_by('tripadvisor_last_update_date','-hotel_id')
		else:
		    print 'check newly added hotels..'
		if len(hotels) > 0:
		    hotel = hotels[0]
		    print 'check for - %s' % (hotel)
		    if hotel.tripadvisor_last_update_date is None:
			...
			hotel.is_tripadvisor_update_running = True
                        ...
                        while (is_task_complete == False):
                            ...
                            if task_hotel.is_tripadvisor_update_running == False:
		        ...
		        time_diff = datetime.now() - (hotel.tripadvisor_last_update_date).replace(tzinfo=None)
