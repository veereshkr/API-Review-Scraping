...
lockfile = '/tmp/scrape_task_google_lock_file'
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        print 'scrape_task_google.py - handle_noargs'
        now = datetime.now()
        print 'now: %s' % now
	...
	try:
		...
		interval = 1
	        hotels = Hotel.objects.filter(is_unavailable=False, is_gmb_enabled=True, is_google_update_running=False).order_by('google_last_update_date','-hotel_id')
	        #hotels = Hotel.objects.filter(is_unavailable=False, is_booking_update_running=False).filter(Q(hotel_id__gt=85)).order_by('booking_last_update_date','-hotel_id')
		if len(hotels) == 0:
		    hotels = Hotel.objects.filter(is_unavailable=False, is_gmb_enabled=True, is_google_update_running=False).order_by('google_last_update_date','-hotel_id')
		else:
		...
		if len(hotels) > 0:
		    ...
		    if hotel.google_last_update_date is None:
			...
			hotel.is_google_update_running = True
                        ...
                        while (is_task_complete == False):
                            ...
                            if task_hotel.is_google_update_running == False:
		        ...
		        time_diff = datetime.now() - (hotel.google_last_update_date).replace(tzinfo=None)
			print '%s - current time_diff: %s' % (datetime.now(), time_diff.total_seconds())
			if time_diff.total_seconds() > 10000:
