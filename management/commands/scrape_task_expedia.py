...
lockfile = '/tmp/scrape_task_expedia_lock_file'
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        print 'scrape_task_expedia.py - handle_noargs'
        now = datetime.now()
        print 'now: %s' % now
	...
	try:
		...
		interval = 1
	        hotels = Hotel.objects.filter(is_unavailable=False, is_expedia_update_running=False, expedia_last_update_date=None).order_by('expedia_last_update_date','-hotel_id')
	        #hotels = Hotel.objects.filter(is_unavailable=False, is_expedia_update_running=False).filter(Q(hotel_id__gt=1)).filter(Q(hotel_id__lte=1)).order_by('expedia_last_update_date','-hotel_id')
	        #hotels = Hotel.objects.filter(is_unavailable=False, is_expedia_update_running=False).filter(Q(hotel_id__gt=85)).order_by('expedia_last_update_date','-hotel_id')
		if len(hotels) == 0:
	            hotels = Hotel.objects.filter(is_unavailable=False, is_expedia_update_running=False).order_by('expedia_last_update_date','-hotel_id')
                else:
                    print 'check newly added hotels..'
		if len(hotels) > 0:
		    hotel = hotels[0]
		    print 'check for - %s' % (hotel)
		    if hotel.expedia_last_update_date is None:
			...
			hotel.is_expedia_update_running = True
                        ...
                        while (is_task_complete == False):
                            ...
                            if task_hotel.is_expedia_update_running == False:
		        ...
		        time_diff = datetime.now() - (hotel.expedia_last_update_date).replace(tzinfo=None)
			print '%s - expedia_last_update_date: %s' % (datetime.now(), (hotel.expedia_last_update_date).replace(tzinfo=None))
	...
	try:
	    ...
	    print '%s - Error in deleting lockfile - %s' % (datetime.now(), str(e))
